...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\NYUAD\\Visual_Imagery\\Experiment\\Visual_Imagery_WM_MEG\\Visual_Imagery_All\\Visual_WM_P_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    WM_P_E = visual.TextStim(win=win, name='WM_P_E',
    ...
    while continueRoutine:
        ...
        # if WM_P_E is starting this frame...
        if WM_P_E.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            WM_P_E.frameNStart = frameN  # exact frame index
            WM_P_E.tStart = t  # local t and not account for scr refresh
            WM_P_E.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(WM_P_E, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'WM_P_E.started')
            # update status
            WM_P_E.status = STARTED
            WM_P_E.setAutoDraw(True)
        
        # if WM_P_E is active this frame...
        if WM_P_E.status == STARTED:
            # update params
            pass
